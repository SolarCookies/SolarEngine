...
void ExtractPage::render(GUI& gui)
{
	...
	if (PKGIndex != -1) {
		...
		//open pkg file 
		std::ifstream pkgfile(pkgFilePaths[PKGIndex], std::ios::binary);
		if (!pkgfile.is_open()) {
			return;
		}
		std::vector<unsigned char> pkgrawdata((std::istreambuf_iterator<char>(pkgfile)), std::istreambuf_iterator<char>());
		pkgfile.close();

		Package CurrentPkg(pkgrawdata, pkgFilePaths[PKGIndex].string());
